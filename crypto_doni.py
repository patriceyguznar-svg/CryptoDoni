...
# -*- coding: utf-8 -*-
"""
CryptoDoni v7 — ИДЕАЛЬНЫЙ ФОРМАТ + БЕЗ КОНФЛИКТОВ
"""
...
async def handle(request):
    return web.Response(text="CryptoDoni v7 — жив!")
...
async def check_wallet(address: str) -> dict:
    ...
    # === USD ===
    trx_usd = result["trx_amount"] * PRICES["TRX"]
    usdt_usd = result["usdt_amount"] * PRICES["USDT"]
    result["total_usd"] = trx_usd + usdt_usd
...
async def check_tron(address: str) -> dict:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url) as resp:
                ...
                if "data" in data and data["data"]:
                    ...
                    # USDT транзакции
                    url_tx = f"https://api.trongrid.io/v1/accounts/{address}/transactions/trc20?limit=5&contract_address={usdt_contract}"
                    async with session.get(url_tx) as resp:
                        data_tx = await resp.json()
                        for tx in data_tx.get("data", []):
                            value = int(tx["value"]) / 1e6
                            to = tx["to"][:8] + "..." if len(tx["to"]) > 8 else tx["to"]
                            time = datetime.fromtimestamp(tx["block_timestamp"] / 1000).strftime('%d.%m %H:%M')
                            txs.append(f"→ {to} | {value:.2f} USDT | {time}")

                    # TRX транзакции
                    url_trx = f"https://api.trongrid.io/v1/accounts/{address}/transactions?limit=5"
    ...
    return {"network": "TRON", "trx_amount": trx_amount, "usdt_amount": usdt_amount, "txs": txs[:3]}  # Только 3 последние
...
async def start(msg: Message):
    await msg.answer(
        "<b>CryptoDoni v7 — ИДЕАЛЬНЫЙ ФОРМАТ!</b>\n\n"
        "Пришли адрес TRON (T...34 символа) — получишь:\n"
        "• Баланс в $ • Последние транзакции • ИИ-анализ"
    )
...
async def handle(msg: Message):
    ...
    try:
        ...
        balance_line = f"TRX: {result['trx_amount']:.2f} (~${trx_usd:.2f}) USDT: {result['usdt_amount']:.2f} (~${usdt_usd:.2f})"
        tx_line = "\n".join(result['txs']) if result['txs'] else "нет"

        text = (
            f"<b>Кошелёк:</b> {short_addr} <b>Сеть:</b> {result['network']} <b>Баланс:</b> {balance_line}\n"
            f"<b>Общая сумма: ~${result['total_usd']:.2f}</b>\n"
            f"<b>Транзакции:</b> {tx_line}\n"
            f"<b>ИИ:</b> {result['ai_analysis']}"
        )
